...
# "전체 수단 비교" 그래프에서 항공 대안으로 볼 수 있게 함. 배출계수(18.71g/km)는
# DEFRA/DESNZ 2025 Ferry(foot passenger) 확정 자료지만, 속도는 목포-제주 1개
# 노선만 역산한 값이라 다른 항로(완도-제주, 여수-제주 등)에는 안 맞을 수 있음.
# ══════════════════════════════════════════════
TRANSPORT = [
    ...
    ("여객선 (페리)",                          18.71,  19,    "선박",     "DEFRA/DESNZ 2025 Ferry(foot passenger) · 속도: 목포-제주 노선(약 85km/4시간30분, Direct Ferries) 🟡 단일 노선 기준 잠정치"),
...
st.caption(
    ...
    "⛴️ 여객선(페리) 배출계수는 DEFRA/DESNZ 2025 확정값이지만, 속도(19km/h)는 목포-제주 1개 노선만 "
    "역산한 잠정치라 다른 항로에는 정확히 맞지 않을 수 있습니다.\n\n"
